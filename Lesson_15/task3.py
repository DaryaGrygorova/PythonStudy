...
class TVController:
    ...
    def __init__(self, channel_list):
        self.channels = {(i + 1): channel for i, channel in enumerate(channel_list)}
        self.current = 1
    ...
    def is_exist(self, search_param):
        """Gets 1 argument - the number N or the string 'name'
        and returns "Yes", if the channel N or 'name' exists in the list,
        or "No" - in the other case"""
        is_exist = "No"
        if isinstance(search_param, str):
            for num, name in self.channels.items():
                if search_param in (num, name):
                    is_exist = "Yes"
        return is_exist
